...
<script type="text/javascript">
	...
	$(function() {
		...
		$.post(url, param, function(data) {
			var vid = "${customer.level.dict_id}";
			//	alert(vid);
			$(data).each(
					function(i, v) {
						if (vid == v.dict_id) {
		...
		$.post(url, param, function(data) {
			var vid = "${customer.source.dict_id}";
			//	alert(vid);
			$(data).each(
					function(i, v) {
						if (vid == v.dict_id) {
...
<BODY>
	<FORM id=form1 name=form1
		...
		<input type="hidden" name="cust_id" value="${customer.cust_id }" /> <input
			type="hidden" name="filepath" value="${customer.filepath }" />
